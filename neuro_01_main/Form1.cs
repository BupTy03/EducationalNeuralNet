...
//      вектор обучения: А соотв.вектору (1;0;0); Б соотв.вектору (0;1;0)
            ...
            {
                for (int j = 1; j <= nn; j++) { dt[j, it] = 0; if (j == it) { dt[j, it] = 1;} }
            }
...
//
            for (int i = 0; i <= 9; i++) // 
            {
                for (int j = 0; j <= 6; j++)
                {
                    ...
                    Ad[i + 1, j + 1] = 0; if (tt1=="1") {Ad[i + 1, j + 1] = 1;};
                    Bd[i + 1, j + 1] = 0; if (tt2=="1") {Bd[i + 1, j + 1] = 1;};
                }
            }
//            string writePath = @"d:\Trading\Via SpecuLatio\SL_Instrum\" + TikerI[ii] + ".txt";
            ...
            {
                textFile.WriteLine(" ");
                for (int j = 1; j <= mc; j++) { textFile.Write(Ad[i, j]); textFile.Write(" "); }
            }
            textFile.WriteLine(" "); textFile.WriteLine(" "); textFile.Write(" dt:  ");
            for (int j = 1; j <= nn; j++) { textFile.Write(dt[1, j]); textFile.Write(" "); }
            textFile.WriteLine(" ");
            textFile.WriteLine(" Б ");
            for (int i = 1; i <= mr; i++)
            {
                textFile.WriteLine(" ");
                for (int j = 1; j <= mc; j++) { textFile.Write(Bd[i, j]); textFile.Write(" "); }
            }
            textFile.WriteLine(" "); textFile.WriteLine(" "); textFile.Write(" dt:  ");
            for (int j = 1; j <= nn; j++) { textFile.Write(dt[2, j]); textFile.Write(" "); }
            textFile.WriteLine(" ");
            textFile.Close();
//            MessageBox.Show(" OK ");
...
//
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            int ix = 0;
            for (int i = 1; i <= mr; i++)
            { for (int j = 1; j <= mc; j++) { ix++; x[ix, 1] = Ad[i, j]; x[ix, 2] = Bd[i, j]; } }
            ...
            for (int i = 1; i <= mx; i++)
            { 
              textFile.Write(x[i, 1]); textFile.Write("  "); textFile.Write(x[i, 2]); textFile.WriteLine(""); 
            }


            //   матрица весов
            float eta = 1;
            for (int i = 1; i <= mx; i++) { for (int j = 1; j <= nn; j++) 
            { w1[i, j] = Convert.ToSingle(Math.Pow(-1,(i+j))); dw1[i, j] = 0;} }
            textFile.WriteLine(" W1 ");
            for (int i = 1; i <= mx; i++)
            {
                textFile.WriteLine("");
                for (int j = 1; j <= nn; j++) { textFile.Write(w1[i, j]); textFile.Write("  ");   }
            }

            //
            int iterm = 10; int Flag = 0;
            textFile.WriteLine("  ");
            textFile.WriteLine("  Education neuronet ");
            for(int iter = 1; iter<=iterm; iter++)
            {
                Flag = 0; textFile.WriteLine(" iter="+Convert.ToString(iter));
                // calc y
                for (int it = 1; it <= tn; it++)
                { // Image
                    ...
                    for (int j = 1; j <= nn; j++)
                    {
                        float s0 = 0; for (int i = 1; i <= mx; i++) { s0 = s0 + x[i, it] * w1[i, j]; }
                        y[j, it] = helpfun.press(s0);
                        textFile.Write(y[j, it]); textFile.Write("  "); 
                    }
                    textFile.WriteLine("");
                    textFile.WriteLine("");
                } // Image
                ...
                for (int i = 1; i <= mx; i++)
                {
                    for (int j = 1; j <= nn; j++)
                    {
                        ...
                        if (i == 35)
                        {
                            textFile.Write(" i= "); textFile.Write(i); textFile.Write("  ");
                            textFile.Write(" j= "); textFile.Write(j); textFile.Write("  ");
                            textFile.Write(" dw1= "); textFile.Write(dw1[i, j]); textFile.Write("  ");
                            textFile.Write(" w1= "); textFile.Write(w1[i, j]); textFile.Write("  ");
                            textFile.Write(" tol= "); textFile.Write(tol); textFile.Write("  ");
                            textFile.Write(" error= "); textFile.Write(error); textFile.Write("  ");
                            textFile.Write(" Flag= "); textFile.Write(Flag); textFile.Write("  ");
                            textFile.WriteLine("");
                        }
                    }  // j  cycle    
                }   // i cycle
                ...
                for (int i = 1; i <= mx; i++)
                {
                    for (int j = 1; j <= nn; j++) { w1[i, j] = w1[i, j] + dw1[i, j]; }
                }
                if (Flag == 0) { iterm = iter;  break; }
            }  // iter

            
            textFile.WriteLine(" ");
            textFile.Write(" iter= "); textFile.Write(iterm); textFile.Write("  ");
            textFile.Write(" Flag= "); textFile.Write(Flag); textFile.WriteLine("");
            textFile.Close();
        ...
        // Распознование нечетко написанных букв
            ...
            for (int i = 0; i <= 9; i++) // 
            {
                ...
                {
                    Bdf[i + 1, j + 1] = 0;
                    if (comboBox1.SelectedIndex == 0) { rr = Ad[i + 1, j + 1]; }
                    if (comboBox1.SelectedIndex == 1) { rr = Bd[i + 1, j + 1]; }
                    if (rr == 1)
                    { this.dataGridView3.Rows[i].Cells[j].Value = Convert.ToString(rr); }
                    else { this.dataGridView3.Rows[i].Cells[j].Value = Convert.ToString(" "); }
                }
            }
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i <= 9; i++) // 
            {
                for (int j = 0; j <= 6; j++)
                {
                    ...
                    Bdf[i + 1, j + 1] = 0; if (tt1 == "1") { Bdf[i + 1, j + 1] = 1; };
                }
            }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            int ix = 0;
            for (int i = 1; i <= mr; i++)
            { for (int j = 1; j <= mc; j++) { ix++; xf[ix] = Bdf[i, j];  } }
            string str = ""; 
            // calc y
                for (int j = 1; j <= nn; j++)
                {
                    float s0 = 0; for (int i = 0; i <= mx; i++) { s0 = s0 + xf[i] * w1[i, j]; }
                    yf[j] = helpfun.press(s0);
                    decimal yfj = Convert.ToDecimal(yf[j]);
                    string st1=Convert.ToString(yfj); st1 = st1.Substring(0, 4);
                    str = str + st1 + "  ";
                }
                textBox5.Text = str;
                if ((yf[1] >= 0.8) & (yf[2] <= 0.2) & (yf[3] <= 0.2)) { goal = "А"; }
                if ((yf[1] <= 0.2) & (yf[2] >= 0.8) & (yf[3] <= 0.2)) { goal = "Б"; }
                textBox4.Text = goal;
        }
